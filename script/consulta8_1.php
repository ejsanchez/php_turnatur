...
//    echo '<p>Clave de cotizaci&oacute;n: '.$ticker_symbol['ticker_symbol'].'</p>';
    echo '<p>Raz&oacute;n social: '.$ticker_symbol['corporate_name'].'</p>';
//    echo '<p>P&aacutegina web: <a href="'.$ticker_symbol['web_page'].'">'.$ticker_symbol['web_page'].'</a></p>';
    ...
    $csv.='"Clave de cotización: '.$ticker_symbol['ticker_symbol'].'"'.$csv_end;
    $csv.='"Razón social: '.$ticker_symbol['corporate_name'].'"'.$csv_end.$csv_end;
}
...
if ($resultado && $registros > 0) {
    ...
    echo "<p>NOTA: Las cantidades son presentadas son valores nominales y en miles de pesos</p>";
    ...
    $csv.='"Año"'.$csv_sep.'"Clave antigua"'.$csv_sep.'"Clave nueva ó Referencia INEGI"'.$csv_sep.'"Cuenta"'.$csv_sep.'"Monto"'.$csv_end.$csv_end;
    ...
    while($row = mysqli_fetch_assoc($resultado)){
	  ...
	  $csv.='"'.$row['report_year'].'"'.$csv_sep.'"'.$row['key_word'].'"'.$csv_sep.'"'.$row['reference'].'"'.$csv_sep.'"'.$row['description_es'].'"'.$csv_sep.'"'.$row['amount'].'"'.$csv_end;
