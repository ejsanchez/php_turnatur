...
$statement_name = "
	SELECT description FROM financial_statements as f, accounts as a
	WHERE a.id = $account
...
if ($resultado && $registros > 0) {
	...
	<td>Cuenta / Subcuenta</th>
	<td>Monto</th>
	</tr><tr>';

   $csv.='"Clave antigua BMV"'.$csv_sep.'"Clave nueva ó Referencia INEGI"'.$csv_sep.'"Cuenta / Subcuenta"'.$csv_sep.'"Monto"'.$csv_end;
   ...
   echo "No se encontraron datos para esta consulta";
}
